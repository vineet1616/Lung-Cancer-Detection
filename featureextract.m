function Result = featureextract(him, m, A, Eigenfaces)
% Recognizing step....
%
...
% InputImage = imread(him);
% InputImage=imresize(him,[200 180]);
% temp = InputImage(:,:,1);

%  temp = rgb2gray(him);
%  [irow icol] = size(him);
%  InImage = reshape(him',irow*icol,1);
Difference = double(him)-m; % Centered test image
ProjectedTestImage = Eigenfaces'*Difference; % Test image feature vector

Result = ProjectedTestImage;
load Net;
X = sim(Net,Result);
