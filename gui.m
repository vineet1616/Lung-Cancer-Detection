...
[train,test] = crossvalind('HoldOut',groups);
cp = classperf(groups);
svmStruct4 = fitcsvm(data(train,:),groups(train),'showplot',false,'kernel_function','quadratic');
classes4 = ClassificationSVM(svmStruct4,data(test,:),'showplot',false);
...
if result1==1
       set(handles.text1,'string','Not Detected NB Initial Stage');
else
       set(handles.text1,'string','Detected NB Final Stage');
...
figure, imshow(img2);title('Detected Image');
...
if result==1
       set(handles.text1,'string','Not Detected Initial Stage');
       
else
       set(handles.text1,'string','Detected Final Stage');
